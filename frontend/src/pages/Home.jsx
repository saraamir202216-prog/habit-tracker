  ...
  return (
    <svg viewBox="0 0 420 380" className="hero-illustration" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      ...
      <g transform="translate(60,60)">
        <rect x="0" y="0" width="230" height="230" rx="22" fill="var(--surface)" stroke="var(--border)" strokeWidth="2" />
        <text x="20" y="38" fontFamily="Poppins" fontWeight="700" fontSize="16" fill="var(--ink)">June</text>
        {Array.from({ length: 7 }).map((_, col) => (
          <text key={col} x={20 + col * 28} y="62" fontFamily="Inter" fontSize="10" fill="var(--ink-muted)" textAnchor="middle">
            {["M", "T", "W", "T", "F", "S", "S"][col]}
          </text>
        ))}
        {Array.from({ length: 4 }).map((_, row) =>
          Array.from({ length: 7 }).map((_, col) => {
            const filled = (row * 7 + col) % 3 === 0 || (row + col) % 5 === 0;
            return (
              <rect
                key={`${row}-${col}`}
                x={14 + col * 28}
      ...
      <g transform="translate(255,205)">
        <rect x="0" y="0" width="130" height="90" rx="18" fill="var(--surface)" stroke="var(--border)" strokeWidth="2" />
        <text x="16" y="30" fontFamily="Inter" fontWeight="600" fontSize="12" fill="var(--ink-muted)">Streak</text>
        <text x="16" y="60" fontFamily="Poppins" fontWeight="800" fontSize="26" fill="var(--ink)">7 days</text>
        <circle cx="110" cy="20" r="16" fill="var(--pine, var(--lime))" />
        <path d="M103 20 L108 25 L118 13" stroke="white" strokeWidth="2.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
      </g>
  ...
  return (
    <div className="calendar-demo">
      ...
      <div className="calendar-demo-grid">
        {days.map((d) => (
          <span className="calendar-demo-daylabel" key={d}>{d}</span>
        ))}
        {seed.map((v, i) => {
          const level = v % 5;
          return <span key={i} className={`calendar-demo-cell level-${level}`} />;
        })}
      </div>
      ...
      <section className="home-features">
        <Link to={user ? "/habits" : "/register"} className="home-feature">
          ...
          <div className="home-feature-body">
            ...
            <p>Daily, specific weekdays, or a weekly target - you choose what works.</p>
          </div>
      ...
      <section className="how-it-works-section">
        <div className="how-it-works-steps">
          ...
          <div className="how-steps-line">
            ...
            <div className="how-step-row">
              <span className="how-step-num how-step-num-lime">2</span>
              <span className="how-step-icon how-step-icon-lime">✓</span>
              <div>
                <h3>Mark it done</h3>
            ...
            <div className="how-step-row">
              <span className="how-step-num how-step-num-coral">3</span>
              <span className="how-step-icon how-step-icon-coral">↗</span>
              <div>
