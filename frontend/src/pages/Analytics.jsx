...
export default function Analytics() {
      ...
      try {
        ...
        const entries = await Promise.all(
          res.data.habits.map(async (h) => {
            ...
            return [h._id, logRes.data.dates];
          })
        );
        setLogsByHabit(Object.fromEntries(entries));
      } catch (err) {
    ...
    return (
      <div>
        ...
        <div className="empty-state-block">
          ...
          <p>Create a habit and log a few days - analytics will show up here automatically.</p>
          <Link to="/habits" className="btn btn-primary">+ New habit</Link>
        </div>
      </div>
    );
  }
  ...
  const best = habits.reduce((a, b) => (b.longest_streak > (a?.longest_streak || 0) ? b : a), null);

  const needsAttention = habits.filter((h) => h.pendingMissedDate);
  ...
  const maxCount = Math.max(1, ...last14.map((d) => countsByDate[d] || 0));

  const sortedHabits = [...habits].sort((a, b) => b.current_streak - a.current_streak);
    ...
    <div>
      ...
      <div className="stats-strip">
        ...
        <div className="stat-block stat-block-lime">
          <span className="stat-value">{activeStreaks}/{habits.length}</span>
          <span className="stat-label">Active streaks</span>
        </div>
        <div className="stat-block stat-block-coral">
          <span className="stat-value">{best ? best.longest_streak : 0}</span>
      ...
      {needsAttention.length > 0 && (
        <div className="grace-banner">
          ⏳ {needsAttention.length} habit{needsAttention.length > 1 ? "s" : ""} in grace period right now:{" "}
          {needsAttention.map((h) => h.name).join(", ")}
        </div>
      )}
      ...
      <div className="card">
        ...
        <div className="analytics-bars">
          {last14.map((dateStr) => {
            ...
            return (
              <div className="analytics-bar-col" key={dateStr}>
                <div className="analytics-bar-track">
                  <div
                    className="analytics-bar-fill"
                    style={{ height: `${Math.max(heightPct, count > 0 ? 8 : 0)}%` }}
                    title={`${dateStr}: ${count} completion${count === 1 ? "" : "s"}`}
                  />
                </div>
                <span className="analytics-bar-label">{dateStr.slice(8)}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="card">
        ...
        <div className="analytics-table">
          ...
          {sortedHabits.map((h) => (
            <div className="analytics-table-row" key={h._id}>
              <span className="analytics-table-name">
                <Link to={`/habits/${h._id}`}>{h.name}</Link>
              </span>
              <span>{scheduleLabel(h)}</span>
              <span>🔥 {h.current_streak}</span>
              <span>🏆 {h.longest_streak}</span>
              <span>{(logsByHabit[h._id] || []).length}</span>
            </div>
          ))}
        </div>
      </div>
