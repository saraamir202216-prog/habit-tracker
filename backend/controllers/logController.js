...
async function markDone(req, res) {
  try {
    const habit = await getOwnedHabit(
      req.params.habitId,
      req.userId
    );
    ...
    if (completed_date > todayStr()) {
      return res.status(400).json({
        message: "Cannot log a future date",
      });
    }
...
/*
 * Get ALL logs for the logged-in user in ONE query.
 * This is used by Analytics and Calendar.
 */
async function getAllUserLogs(req, res) {
  try {
    ...
    res.status(500).json({
      message: "Failed to load all logs",
      error: err.message,
    });
  }
}
